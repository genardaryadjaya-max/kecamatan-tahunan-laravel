    ...
    <div class="container-fluid px-4 py-8">
        <div class="max-w-4xl mx-auto bg-[#1e281e]/60 border border-white/5 rounded-xl overflow-hidden shadow-lg p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-white">Edit Statistik</h1>
                <a href="{{ route('admin.statistik.index') }}" class="text-white/60 hover:text-white transition-colors">
            ...
            <form action="{{ route('admin.statistik.update', $statistik) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category" required
                            ...
                            @foreach(['penduduk' => 'Penduduk', 'pertanian' => 'Pertanian', 'kesehatan' => 'Kesehatan', 'pendidikan' => 'Pendidikan', 'ekonomi' => 'Ekonomi', 'infrastruktur' => 'Infrastruktur'] as $val => $label)
                                <option value="{{ $val }}" class="bg-[#1e281e]" {{ old('category', $statistik->category) == $val ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="year" value="{{ old('year', $statistik->year) }}" min="2000" max="2100"
                            required
