...
<div class="container-fluid px-4 py-8">
    <div class="max-w-2xl mx-auto bg-[#1e281e]/60 border border-white/5 rounded-xl p-6 shadow-lg">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-xl font-bold text-white">Tambah Anggota Struktur</h1>
        ...
        <form action="{{ route('admin.struktur.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="position" required
                        ...
                        <option value="Camat" {{ old('position') == 'Camat' ? 'selected' : '' }}>Camat</option>
                        <option value="Sekretaris Camat" {{ old('position') == 'Sekretaris Camat' ? 'selected' : '' }}>Sekretaris Camat</option>
                        <option value="Kepala Sub Bagian Umum dan Kepegawaian" {{ old('position') == 'Kepala Sub Bagian Umum dan Kepegawaian' ? 'selected' : '' }}>Kepala Sub Bagian Umum & Kepegawaian</option>
                        <option value="Kepala Sub Bagian Perencanaan dan Keuangan" {{ old('position') == 'Kepala Sub Bagian Perencanaan dan Keuangan' ? 'selected' : '' }}>Kepala Sub Bagian Perencanaan & Keuangan</option>
                        <option value="Kepala Seksi Tata Kepemerintahan" {{ old('position') == 'Kepala Seksi Tata Kepemerintahan' ? 'selected' : '' }}>Kepala Seksi Tata Kepemerintahan</option>
                        <option value="Kepala Seksi Ketentraman, Ketertiban Umum dan Pengamanan Pantai" {{ old('position') == 'Kepala Seksi Ketentraman, Ketertiban Umum dan Pengamanan Pantai' ? 'selected' : '' }}>Kepala Seksi Ketentraman & Ketertiban Umum</option>
                        <option value="Kepala Seksi Pemberdayaan Masyarakat Desa dan Perekonomian" {{ old('position') == 'Kepala Seksi Pemberdayaan Masyarakat Desa dan Perekonomian' ? 'selected' : '' }}>Kepala Seksi Pemberdayaan Masyarakat Desa</option>
                        <option value="Kepala Seksi Sosial dan Lingkungan Hidup" {{ old('position') == 'Kepala Seksi Sosial dan Lingkungan Hidup' ? 'selected' : '' }}>Kepala Seksi Sosial & Lingkungan Hidup</option>
                        <option value="Kepala Seksi Pelayanan Umum" {{ old('position') == 'Kepala Seksi Pelayanan Umum' ? 'selected' : '' }}>Kepala Seksi Pelayanan Umum</option>
                        <option value="Jabatan Fungsional" {{ old('position') == 'Jabatan Fungsional' ? 'selected' : '' }}>Jabatan Fungsional</option>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white focus:border-gold/50 outline-none">{{ old('description') }}</textarea>
            </div>
